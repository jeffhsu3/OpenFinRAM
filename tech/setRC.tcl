# ASAP7 routing RC used by the OpenROAD-flow-scripts ASAP7 platform.
# The upstream capacitances are quoted in fF/um, while these Liberty files set
# OpenSTA's command capacitance unit to pF.  Scale capacitance by 1e-3 here so
# standalone OpenFinRAM runs do not silently create a 1000x RC error.  The
# resistance coefficients already match the Liberty kOhm/um command units.
# Keep this local copy with the LEF so runs never fall back to zero wire RC.
# Upstream: flow/platforms/asap7/setRC.tcl

set_layer_rc -layer M1 -resistance 7.04175E-02 -capacitance 1e-13
set_layer_rc -layer M2 -resistance 2.97127E-02 -capacitance 1.74942E-04
set_layer_rc -layer M3 -resistance 3.12870E-02 -capacitance 1.55554E-04
set_layer_rc -layer M4 -resistance 1.80365E-02 -capacitance 1.78475E-04
set_layer_rc -layer M5 -resistance 1.89935E-02 -capacitance 1.64264E-04
set_layer_rc -layer M6 -resistance 1.18796E-02 -capacitance 1.87426E-04
set_layer_rc -layer M7 -resistance 1.25096E-02 -capacitance 1.62979E-04
set_layer_rc -layer M8 -resistance 8.44765E-03 -capacitance 1.03962E-04
set_layer_rc -layer M9 -resistance 8.89556E-03 -capacitance 9.28446E-05

set_wire_rc -signal -resistance 2.65684E-02 -capacitance 1.65790E-04
set_wire_rc -clock  -resistance 2.14161E-02 -capacitance 1.45426E-04

set_layer_rc -via V1 -resistance 1.72E-02
set_layer_rc -via V2 -resistance 1.72E-02
set_layer_rc -via V3 -resistance 1.72E-02
set_layer_rc -via V4 -resistance 1.18E-02
set_layer_rc -via V5 -resistance 1.18E-02
set_layer_rc -via V6 -resistance 8.20E-03
set_layer_rc -via V7 -resistance 8.20E-03
set_layer_rc -via V8 -resistance 6.30E-03
set_layer_rc -via V9 -resistance 6.30E-03
